...
- Замена всего кластера целиком на новый (типа Blue/Green стратегии). Без риска, с возможностью отката. Требуется: 
...
- Замена узла на новый (типа RollingUpdate стратегии) - без риска, с возможностью отката
- Обновление каждого узла (с запретом планирования/выведением нагрузки) - есть риск, сложность отката

Применим 3-й подход. Будем обновлять каждый узел по порядку, начиная с узлов Control Plane (роль мастера). Выполняем для каждой ноды:
